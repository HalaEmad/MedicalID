dsf.controller('adiboffersController', function($scope, $rootScope, $state,
	...
	$scope.$parent.PageHeaderParam = "ADIB Offers";
	 ...
	 if(WL.Client.isConnected()==true)
		{
		 WL.Device.getNetworkInfo (function(deviceStatus){
				...
				{
					$scope.offerListUpdated =($.makeArray(JSON.parse(localStorageService.getADIBOfferList()))).slice(0,100);
	...
	$scope.appendmore=function()
	{
		...
		if(roundNo>0)
			{
			tempArray=$.makeArray(JSON.parse(localStorageService.getADIBOfferList())).slice(((roundNo+1)*100+1),(roundNo+1)*100+100);
			...
			if(localStorageService.getGlobalDataRefreshLastTimeStamp()==null)
			{
			 ...
			 tempArray=$.makeArray(JSON.parse(localStorageService.getADIBOfferList())).slice(101,200);
				offerModel.setDSFOffersAddMoreCount(roundNo+1);
			}
